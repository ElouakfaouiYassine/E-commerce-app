...
class GamesCategoryFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val  view:View = inflater.inflate(R.layout.fragment_games_category, container, false)
        recyclerView = view.findViewById(R.id.rv_games_category)
    ...
    private fun loadgamesProducts() {
        val gamesProductsCursor = databaseHelper.getProductsByCategory("Games")
        val gamesProductsList = mutableListOf<Products>()

        gamesProductsCursor?.use { cursor ->
            ...
            while (cursor.moveToNext()) {
                ...
                gamesProductsList.add(product)
            }
        }

        adapter.updateData(gamesProductsList)
        gamesProductsCursor?.close()
    }
